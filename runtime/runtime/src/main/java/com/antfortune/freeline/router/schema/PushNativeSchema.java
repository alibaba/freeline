...
public class PushNativeSchema implements ISchemaAction {
    ...
    public void handle(String method, String path, HashMap<String, String> headers, Map<String, String> queries, InputStream input, EmbedHttpServer.ResponseOutputStream response) throws Exception {
        Log.e("LongLinkIncrementServer","method "+method+","+path+headers+queries);
        ...
        try {
            ...
            while ((ze = zis.getNextEntry()) != null) {
                ...
                com.antfortune.freeline.FreelineCore.updateDynamicTime();
                Log.d("LongLinkIncrementServer", new StringBuilder().append("native file ").append(filename).append("received (").append(file.length()).append(" bytes)").toString());
            }
        } catch (Exception ex){
            ex.printStackTrace();
        } finally {
